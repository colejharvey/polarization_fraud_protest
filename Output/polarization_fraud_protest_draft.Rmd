...
Rising political polarization has been cited as one of the primary causes of this wave of autocratization [@haggardAnatomyDemocraticBacksliding2021;@mccoyPolarizationGlobalCrisis2018;@mccoyTheoryPerniciousPolarization2019;@Laebens2020;@vanlitCounteringAutocratizationRoadmap2023]. There are several proposed mechanisms that link the two concepts. When society is polarized into two, mutually distrustful camps, incumbents' incentive to voluntarily relinquish power in elections is diminished. When an opposing group is viewed as highly threatening, either to one's own group or toward the political order generally, increasingly undemocratic actions by the incumbent government can be tolerated by its supporters. 

Election manipulation offers one such undemocratic tool. Illegal techniques like voter intimidation and fraud can be used to increase the incumbent's vote-share mechanically on election day. Alternatively, legal mechanisms like gerrymandering, ballot restrictions, and media control can give the incumbent structural advantages that are difficult for the opposition to overcome. Polarization, which has been argued to increase the risk of autocratization as a general phenomenon, may also contribute to the erosion of election integrity as a mechanism.
...
By strengthening support for the partisan in-group and intensifying dislike and fear of the out-group, deeper polarization can increase support for democratic backsliding [@orhanRelationshipAffectivePolarization2022]. As a result, people may be willing to trade off democratic values, even if they genuinely support them, to avoid having the opposing side take power [@svolikPolarizationDemocracy2019a;@graham_svolik_2020;@aarslewWhyDonPartisans2022;@kingzetteHowAffectivePolarization2021]. Citizens may come to view their counterparts on the opposing side in exaggerated, stereotypical forms [@levenduskyAmericansNotPartisans2018], and to perceive them as more serious threats to democracy as a result [@krishnarajanRationalizingDemocracyPerceptual2023;@moore_bergExaggeratedMetaperceptionsPredict2020;@braleyWhyVotersWho2023]. This division of society into opposing camps for whom elections become zero-sum games, in turn, makes democratic violations by the incumbent seem increasingly acceptable to their supporters [@arbatliUnitedWeStand2021;@simonovitsDemocraticHypocrisyOutGroup2022]. 

While several studies have shown that increasing ideological polarization makes individuals more likely to support un-democratic candidates [@graham_svolik_2020;@svolikWhenPolarizationTrumps2020], cross-national analysis suggests that affective polarization is most dangerous for democracy [@orhanRelationshipAffectivePolarization2022;@mccoyPolarizationGlobalCrisis2018]. Affective polarization, the tendency for members of an in-group to view the outgroup with animosity [@Iyengar2019], can lead people to view out-partisans as threatening, untrustworthy, and even as illegitimate participants in the democratic process [@mccoyTheoryPerniciousPolarization2019]. 

As a result, when affective polarization is high, incumbents (and their supporters) should have more to fear from losing office. Not only will policy likely move away from their ideal point, asin any electoral loss, but they will also be subject to government by a group perceived as threatening and perhaps even unfit to govern.  

Incumbents should thus be more likely to rely on election manipulation to maintain power in polarized settings, where they feel they have more to lose if the opposition wins. This effect is likely in part psychological---fear, anxiety, and anger toward the threatening group are closely connected to affective polarization [@renstromThreatsEmotionsAffective2023]---and in part strategic. In a highly polarized society, the incoming government likely views the outgoing party with the same sense of fear and hostility; as a result, they may be expected to take steps to consolidate its rule. This 'fear effect' may drive increases in manipulation as polarization increases, as is shown in formal modeling by Horz [-@horzElectoralManipulationPolarized2021]. This assumption is also well-grounded in the literature on authoritarian politics, which often holds that the more incumbents stand to lose from defeat, the more vigorously they will defend their positions [@baturoDemocracyDictatorshipTerm2014;@fergussonRealWinnerCurse2021]. As a result, when societies are highly polarized, there are strong incentives for incumbents to grab more power where they can. This effect has two sides: first, the incumbent has incentives to keep opponents out of power, and second, they are likely to have more latitude from their own supporters for violating democratic in the interests of keeping power [@svolikWhenPolarizationTrumps2020].  Crucially, this effect should operate separately of the ideological distance between the two groups.^[For instance, two political groups could be ideologically close, yet highly affectively polarized along other social dimensions [@harteveldTickingAllBoxes2021].]
...
Since much of the prior work in this area has been conducted in the formal modeling tradition, a formal model is used here to demonstrate these dynamics more clearly. The model is deliberately simple and conservative, and aimed at producing empirical hypotheses under two assumptions: affective polarization is distinct from the policy gains of holding office, and electoral popularity is distinct from post-election strength during a contest in the streets.
...
In the figure describing the extensive form of the game, each terminal node is marked by a letter.  Each player has a direct utility from winning office, which is normalized to 1. Not holding office has a direct payoff of 0. This represents the baseline benefit to the player from holding office and pursuing their policy objectives, and sets a standard by which the cost of conflict and the indirect benefits of winning elections can be measured. 

Each player also has an indirect utility of winning office, derived from keeping the other party out of power $B > 0$. As polarization increases, each player gains a greater indirect benefit from keeping its opponent out of power. At very low levels of polarization, parties only gain a small indirect benefit from holding office, relative to the baseline benefit of pursuing favored policies. As $B$ approaches 1, the utility of keeping the opposing party out of office comes to equal the direct policy benefits of holding power. As it increases in value beyond 1, keeping the opponent out of office becomes the primary benefit of holding power. This framing excludes highly unusual cases in which the indirect benefit of holding office may be negative, implying that a party prefers to see its opponent take office (e.g. some wartime coalition governments). 

If a player wins power, it receives the benefit $1 + B$. When a player ends the game out of power, it receives utility $0$. For this baseline game, polarization is assumed to be symmetric, such that $B$ is common to both players. Lastly, if a protest occurs, this is assumed to be the kind of massive and/or sustained protest campaign that threatens to unseat the incumbent; the outcome of this contest is probabilistic, and both sides pay the common cost $c > 0$ regardless of the outcome. Each player's utility function $u_i$ is captured formally below. A table linking the players' expected utilities to each node in the game is provided in the appendix.

\[
u_i =
\begin{cases}
1 + B - c & \text{if player } i \text{ holds office and protest occurs}, \\
1 + B     & \text{if player } i \text{ holds office and no protest occurs}, \\
- c       & \text{if player } i \text{ loses office and protest occurs}, \\
0         & \text{otherwise.}
\end{cases}
\]
...
While the model includes elements of chance (*p* and *q*), at each node both players are aware of the previous history of the game. Payoffs and probabilities are common knowledge. As a result, the game can be solved using subgame perfect equilibria and backward induction. The opposition has the last move in the game, and in a clean election where the incumbent claims victory, the opposition will choose to protest if its expected utility from challenging the election is greater than that of accepting the results. It will do so if $q(0-c) + (1-q)(1 + B-c) > 0$. Solving this inequality for *q*, the probability that the incumbent will defeat the protesters, yields the following result.

\textbf{Lemma 1 (Opposition protest strategy)}: \textit{In any subgame following an incumbent electoral victory, the opposition protests if and only if}

\begin{equation}
q < \frac{(B - c + 1)}{(B+1)} \equiv q_p 
(\#eq:protest)
\end{equation}

As the inequality shows, in this game the opposition’s optimal protest decision is independent of the incumbent’s choice of electoral manipulation *F*.

Turning to the incumbent's decision to manipulate or not, they can anticipate that the opposition will protest when $q < q_p$, regardless of the incumbent's behavior. If $q > q_p$, and therefore the opposition will not protest, then strategy *F* dominates *~F*. Because fraud increases the incumbent's probability of winning the election, it strictly prefers manipulation if it knows that the electoral stage will be the final stage of the game. If, on the other hand, $q < q_p$ and opposition protest is expected, the incumbent's decision to manipulate (to improve it chance of winning the election) depends on the cost of the conflict and the expected benefit of maintaining office after the protest stage. This yields the following result.

\textbf{Lemma 2}: \textit{If the opposition is expected to protest, the incumbent will engage in manipulation if and only if}
...
As a result of Lemmas 1 and 2, there are four possible equilibria to the game, summarized below.  

\textbf{Proposition 1}: \textit{The game has four unique subgame perfect equilibria, based on the relative ordering of q, $q_f$, and $q_p$. If and only if:}

\begin{align}
 q_f > q > q_p & \quad \text{the incumbent holds clean elections; no protest.} \\
 q  > q_f \quad \& \quad q > q_p  & \quad \text{the incumbent manipulates; no protest.} \\
 q_p > q > q_f & \quad  \text{the incumbent manipulates; the opposition protests.} \\
 q_p > q \quad \& \quad q_f > q & \quad \text{the incumbent holds clean elections; theopposition protests.}
\end{align}


Since the denominator of both critical values of *q* are the same, it is straightforward to see that the relationship between the $q_f$ and $q_p$ hinges on the relationship between the degree of polarization $B$ and the cost of conflict *c*. As the inequalities show, the equilibrium outcome depends on the relationship between *q*, the incumbent's probability of defeating a protest movement and the ratio between the cost of conflict *c* and the degree of polarization *B*. Figure \@ref(fig:formalfig) illustrates these comparative statics under two different scenarios. In the left-hand panel of the figure, the cost of conflict to both parties is relatively low compared to the possible values for polarization. In the right-hand figure, the cost of conflict is relatively high.


As the figure shows, increasing distance between the groups represented by the incumbent and the opposition increases the size of *B*; holding *c* constant, shifting values of *B* affect the range of *q* for which each equilibrium is possible. At low levels of polarization, each player is more willing to give up power to the other. As such, the risk of a costly conflict can prevent manipulation by the incumbent and undemocratic attempts to challenge a clean election by the opposition\textemdash the normatively ideal outcome. The right panel of Figure \@ref(fig:formalfig) illustrates how this democratic outcome can hold for all values of *q*, but only when polarization (*B*) is relatively low and the cost of conflict is high.  

As the level of polarization increases to where it meets the cost of conflict, asymmetric outcomes become more likely. This is illustrated in the portion of the right-hand panel of Figure \@ref(fig:formalfig) where the value of *B* is greater than zero but less than the cost of conflict (the vertical dashed line). In this region, stronger incumbents (q > q_fraud) can engage in manipulation in order to make winning more likely, while the opposition is deterred from protesting by the likelihood of losing a post-election conflict. By contrast, weaker incumbents (q < q_fraud) hold clean elections which the opposition nevertheless tries to challenge if they lose. 

Finally, as polarization increases beyond the cost of conflict (i.e. to the right of the vertical line in both panels of Figure \@ref(fig:formalfig)), mutual confrontation becomes the equilibrium outcome for increasingly wide values of *q*. Extremely strong incumbents can deter protest, and extremely weak incumbents are deterred from manipulation, but for most middle values of incumbent strength manipulation and protest are both optimal strategies. In this equilbrium, both sides are so opposed to the other that they are willing to take significant risks to keep them out of power. The left-hand panel of Figure \@ref(fig:formalfig) illustrates how this is the most common equilibrium strategy in cases where the cost of conflict is low.
...
q_vals <- seq(0, 1, by = .01)

grid <- expand.grid(B = B, q = q_vals)

# Compute thresholds
grid$q_protest <- (grid$B - c + 1)/(grid$B + 1)
grid$q_fraud   <- c/(grid$B + 1)

# Classify equilibrium region
grid <- grid %>%
  mutate(equilibrium = case_when(
    q_fraud > q & q > q_protest ~ "Clean / No Protest",
    q > q_fraud & q > q_protest ~ "Manipulate / No Protest",
    q_protest > q & q > q_fraud ~ "Manipulate / Protest",
    q < q_protest & q < q_fraud ~ "Clean / Protest"
  ))
...
geom_line(data = fake_data,
          aes(x = B, y = q_fraud),
          color = "black", linetype = "dashed", size = .9) +
  ...
  theme_bw() +
  labs(y = "q",
       title = "(a) Low-cost equilibria (c = .25)",
       fill = "Equilibrium") +

  scale_fill_manual(values = c(
  "Clean / No Protest" = "white",
  "Manipulate / No Protest" = "grey75",
  "Manipulate / Protest" = "grey45",
  "Clean / Protest" = "black"
)) + 
...
p_highcost <- ggplot() +
  ...
  # Threshold curves
 geom_line(data = fake_data,
          aes(x = B, y = q_protest, linetype = "q_p(B)"),
          color = "black", size = .9) +

geom_line(data = fake_data,
          aes(x = B, y = q_fraud, linetype = "q_f(B)"),
          color = "black", size = .9) +
  ...
  labels = c(
    expression(q[f](B)), #This labeling order is correct
    expression(q[p](B))
  )
) +
  geom_text(data = data.frame(
...
model.protest.count4 <- glm.nb(n.events.post ~ v2xel_frefair + winner.share.upd + v2cacamps.1lag + presidential + physinteg.1lag   +  urban.pct +  unscheduled_election + polyarchy_noelex.1lag + gdpgro.1lag + n.events.pre, data = vdem_data)
...
modelsummary::modelsummary(models, stars = T, gof_omit = c("F|RMSE"), title = "Negative binomial models of post-election protest", coef_omit = "country", coef_rename = 
                            c( "v2xel_frefair" = "Overall election integrity",
                               ...
                               "v2elintmon" = "Intl. monitors",
                               "n.events.pre" = "N. pre-election protests"),
                           add_rows = felabel)
...
In each of these specifications, affective polarization is associated with an increase in the number of expected post-election protests. The effect is substantively sizable as well as statistically significant. Taking the coefficient for polarization reported in Model 5 (the fixed-effects model), a one-unit increase in affective polarization is associated with a 97% increase in the expected number of post-election protests, on average. A one-unit increase is somewhat less than one standard deviation of the variable (1.22).^[As a benchmark, the United States experienced a one-unit increase in this measure roughly between 1990 and 2009, and another one-unit increase between 2009 and 2016. Brazil (another case of increasing polarization, electoral protest, and allegations of manipulation) sees a roughly one-unit increase between 2008 and 2014, and again between 2014 and 2018.] This effect is persistent across models, even when controlling for the level of election integrity, the degree of democratic openness, the level of repression, the winner's claimed vote-share, and the number of pre-election protests.
...
model_wolf <- glm.nb(protested_clean_count ~ winner.share.upd + gdpgro.1lag  + polyarchy_noelex.1lag  + physinteg.1lag + unscheduled_election + presidential +  v2cacamps.1lag + I(v2cacamps.1lag^2), data = vdem_data )




model_clean <- glm(clean_elex ~ winner.share.upd + polyarchy_noelex.1lag + presidential + physinteg.1lag + gdpgro.1lag  +   unscheduled_election  +v2cacamps.1lag, data = vdem_data)




model_success_base <- glm(clean_no_protest ~ winner.share.upd + gdpgro.1lag  + polyarchy_noelex.1lag + presidential + physinteg.1lag +    unscheduled_election  +v2cacamps.1lag, data = vdem_data %>% filter(gdpgro.1lag < 75)) #Excludes outlier Luxembourg

model_success <- glm(clean_no_protest ~ winner.share.upd + gdpgro.1lag  + polyarchy_noelex.1lag + presidential + physinteg.1lag +    unscheduled_election  +v2cacamps.1lag*gdpgro.1lag, data = vdem_data %>% filter(gdpgro.1lag < 75))

model_dominance <- glm(rigged_no_protest ~ winner.share.upd + gdpgro.1lag  + polyarchy_noelex.1lag + presidential + physinteg.1lag +    unscheduled_election  +v2cacamps.1lag + polsq, data = vdem_data)

model_conflict <- glm.nb(mutual_conflict ~ winner.share.upd + gdpgro.1lag  + polyarchy_noelex.1lag + presidential + physinteg.1lag  +  unscheduled_election  +v2cacamps.1lag, data = vdem_data)
```
...
modelsummary::modelsummary(models, stars = T, title = "Logistic and negative binomial regression models of election outcome", gof_omit = "R2|BIC|F|RMSE",
                           coef_rename = 
                            ...
                            notes = list('Models 6 and 7 are logit models; Models 8 and 9 are negative binomial models'), add_rows = rows) %>% kableExtra::landscape()
```

Hypotheses 2 through 4 refer to specific outcomes of election integrity and protest, referring to the illustrative equilibria depicted in Figure \@ref(fig:formalfig). First, Model 6 reports the relationship between affective polarization and the likelihood of a 'successful deterrence' outcome, in which elections are clean and go unprotested. In this logit model, polarization appears negatively correlated with this outcome, as predicted. Exponentiation of the coefficient for *affective polarization* in Model 6 indicates that each one-unit increase in polarization reduces the likelihood of a clean, uncontested election by approximately 3%. This may appear substantively modest in isolation, but elections are after all repeated events. Consequently, even small reductions in the odds of "successful deterrence" compound over time, substantially decreasing the probability of an unbroken string of democratic elections in more polarized cases. This relationship is statistically significant even when controlling for other factors like the degree of democratic openness and physical integrity rights. Hypothesis 2 is supported.
...
Lastly, returning to Model 7, Hypothesis 5 appears supported\textemdash the effect of polarization on successful deterrence is conditional on the level of economic development. In particular, H5 posits that when economic development is higher, the negative effect of polarization will be more muted. This hypothesis is also supported. Figure \@ref(fig:gdpplot) shows the effect of GDP per capita on a pro-democratic outcome (a clean, un-protest election), conditional on the degree of political polarization. The results are just as predicted. For low-income cases, polarization has no effect: the likelihood of clean, unprotested election is consistently low for all values of polarization. On the other hand, for high per capita income cases, polarization matters. When polarization is low, the likelihood of a successful democratic election is approximately fifty percent. As polarization increases, the likelihood of such a normatively desirable outcome declines significantly to approximately thirty percent. The selected values of GPD per capita reflect one standard deviation above and below the mean. Each point in the figure represents an observation plotted by its GDP per capita and whether or not a clean, unprotested election occurred in that case. These histograms demonstrate common support across most of the range of the *polarization variable*, but also highlight that there are no cases of clean, unprotested elections at the highest levels of polarization in this dataset.^[This plot was produced using the `marginaleffects` package in R [@marginaleffects_pkg].]    
...
gdp_plot <- plot_predictions(model_success, condition = c("v2cacamps.1lag", gdpgro.1lag = list(c(.33, 15.6)))) +
    geom_dots(
        alpha = .8,
        scale = .3,
        pch = 24,
...
gdp_plot + coord_cartesian(xlim = c(-3.5, 3.05), ylim = c(0, 1)) + labs(x = "Polarization (lag)", y = "Predicted value", color = "Log GDP (lag)", fill = "Log GDP (lag)") + theme_bw()
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
p3 <- ggplot(pol_data) + geom_line(aes(x = Year, y = aff_pol)) + facet_wrap(~region)  + labs(y = "Affective polarization") + theme_bw()
```
...
One noteworthy aspect of the model, and its empirical support, is that costly conflict can occur even in the absence of asymmetric information. 'Crying wolf'  scenarios can occur even when the incumbent avoids manipulation, and the 'mutual conflict' scenario can occur even when the incumbent anticipates protests will follow his efforts to rig the election. This is in some ways a conservative approach; adaptations of this model that allow for imperfect information about manipulation would only serve to make normatively bad outcomes more likely, and 'successful deterrence' less likely. This also helps explain some historical instances that are puzzling when viewed from a certain perspective. For instance, Ukraine's Orange Revolution happened in a polarized society, obvious pre-election mobilization by the opposition, and widespread fraud by the incumbent. This is a puzzling outcome if we expect the incumbent to back down in the face of costly protest (and subsequent defeat). Yet if polarization makes electoral loss untenable, incumbent should be expected to go to great lengths to 'gamble for resurrection.'  
...
Also note in appropriate place in text that the model is agnostic about centralized vs. mass protest. The theory should apply in either case, so long as the group is affectively polarized--high polarization makes them more willing to take risks.
...
```{r hurdle, eval=FALSE, echo=FALSE, message=FALSE, include=FALSE}




model.protest.count <- hurdle(n.events.post ~  winner.share.upd + v2cacamps.1lag + presidential + physinteg.1lag   +  urban.pct +  unscheduled_election + polyarchy_noelex.1lag + gdpgro.1lag, data = vdem_data, dist = "negbin", zero.dist = "binomial" )
summary(model.protest.count)

hurdlePart <- glm(formula = I(n.events.post>0) ~ winner.share.upd + v2cacamps.1lag + presidential + physinteg.1lag   +  urban.pct +  unscheduled_election + polyarchy_noelex.1lag + gdpgro.1lag, data = vdem_data,
                  family  = binomial(link = "logit"))
coef(summary(hurdlePart))  #This confirms how to interpret the bottom part of the hurdle summary: the binomial is looking for successes (i.e. protest count > 0) So polarization is associated with greater likelihood of initiation, but not count.

expCoef <- exp(coef((model.protest.count)))
...
model.protest.count2 <- hurdle(n.events.post ~ v2xel_frefair + winner.share.upd + v2cacamps.1lag + presidential + physinteg.1lag   +  urban.pct +  unscheduled_election + polyarchy_noelex.1lag + gdpgro.1lag, data = vdem_data, dist = "negbin", zero.dist = "binomial" )
summary(model.protest.count2)

model.protest.count3 <- hurdle(n.events.post ~ v2xel_frefair + winner.share.upd + v2cacamps.1lag + presidential + physinteg.1lag   +  urban.pct +  unscheduled_election + polyarchy_noelex.1lag + gdpgro.1lag, data = vdem_data  %>% filter(inc.lose == 0), dist = "negbin", zero.dist = "binomial")
summary(model.protest.count2)


model.protest.count4 <- hurdle(n.events.post ~ v2xel_frefair + winner.share.upd + v2cacamps.1lag + presidential + physinteg.1lag   +  urban.pct +  unscheduled_election + polyarchy_noelex.1lag + gdpgro.1lag + n.events.pre, data = vdem_data, dist = "negbin", zero.dist = "binomial")
summary(model.protest.count4)
```
