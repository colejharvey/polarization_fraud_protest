...
Since much of the prior work in this area has been conducted in the formal modeling tradition, a formal model is used here to demonstrate these dynamics more clearly, and to demonstrate different predictions. The model is deliberately simple and conservative, and aimed at producing empirical hypotheses if electoral popularity and post-election strength are formally decoupled and affective polarization influences parties' incentives for holding office.
...
The opposition’s optimal protest decision is independent of the incumbent’s choice of electoral manipulation *F*.

Turning to the incumbent's decision to manipulate or not, they can anticipate that the opposition will protest when $q < q_p$, regardless of the incumbent's behavior. If the incumbent manipulates, it increases its probability of winning the election by $ f > 0$ and faces the same post-election contest as after a clean victory, conditional on the value of *q*. Manipulation is therefore optimal whenever the expected benefit of increasing the probability of reaching the post-election contest outweighs the expected cost of protest. Assuming that $b_i > -1$, which is true by construction, this yields the following result.

\textbf{Lemma 2}: \textit{The incumbent will engage in manipulation if and only if}
...
\textbf{Proposition 1}: \textit{When the value of holding office B is greater than -1, the game has four unique subgame perfect equilibria, based on the relative ordering of q, $q_f$, and $q_p$. If and only if:}
...
As the figure shows, increasing distance between the groups represented by the incumbent and the opposition increases the size of *B*; holding *c* constant, shifting values of *B* affect the range of *q* for which each equilibrium is possible. (As a reminder, *q* refers to the probability that the incumbent will prevail in an extra-electoral confrontation.) At low levels of polarization, each player is more willing to give up power to the other. As such, the risk of a costly conflict can prevent manipulation by the incumbent and undemocratic attempts to challenge a clean election by the opposition\textemdash the normatively ideal outcome. The right panel of Figure \@ref(fig:formalfig) illustrates how this democratic outcome can hold for all values of *q*, but only when polarization (*B*) is relatively low and the cost of conflict is high.  
...
```{r protestcount, fig.cap="Histogram of post-election protest events", echo=FALSE, message=FALSE, warning=FALSE}
ggplot(vdem_data, mapping = aes(x = n.events.post)) +
  geom_histogram(binwidth = 1) +
  labs(x = "N. protest events") + theme_bw()
```

```{r vuong, echo=FALSE, message=FALSE}
