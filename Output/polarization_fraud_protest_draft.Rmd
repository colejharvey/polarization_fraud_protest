...
A simple formal model is used to demonstrate these dynamics more clearly, when when the opposition is modeled as a unitary strategic actor. The structure of the game is as follows. The game has two players: the incumbent government, and an opposition. The incumbent chooses whether to hold a clean election (F = 0) or engage in some degree of electoral manipulation (F = 1). If the incumbent chooses to hold a clean election, it wins the election with probability *p* (0 < p < 1). If it chooses to manipulate the election, it wins with probability *p + f*, where *f* > 0 and *p + f* <= 1. At the next stage, the election winner is revealed. If the incumbent wins, the opposition may choose to protest or to accept the results. If the opposition stages protests, the incumbent succeeds in quashing them with probability *q*, while the opposition ousts the government with probability 1-*q* If the incumbent loses the election, the game ends there.

The players receive the following payoffs. Each player has a utility from winning b_w > 0 and a disulity from losing to the other party b_l <= 0. If a player wins, it receives the benefit B = b_w - b_l. This is meant to capture two dimensions of polarization. The first term captures the policy benefits of holding power. The second term captures the affective antipathy toward the other side. That is, each player gains some utility from winning power and some utility from keeping the other party out of power. In societies where negative partisanship is high b_w will be less than |b_l|: keeping the other side out of power is the primary benefit to winning elections. When a player ends the game out of power, it receives utility -B. This represents lost utility from policy changes carried out by the other side, as well as any negative affective utility from seeing the other side in power. For this baseline game, polarization is assumed to be symmetric, such that B and -B are common to both players. Lastly, if a protest occurs, both sides pay the common cost *c* > 0.

While the model includes elements of chance (*p* and *q*), at each node both players are aware of the previous history of the game. Payoffs and probabilities are common knowledge. As a result, the game can be solved using subgame perfect equilibria and backward induction. The opposition has the last move in the game, and in a clean election where the incumbent claims victory, the opposition will choose to protest if its expected utility from challenging the election is greater than that of accepting the results. It will do so if $pq(-B-c) + p(1-q)(B-c) > p(-B)$. Solving this inequality for *q* yields the result that when F = 0 and W = 1, the opposition will protest when $$q < q* = 1-c/(2B)$$. 

Turning to the subgame where F = 1 and W = 1 (that is, where the incumbent won and engaged in manipulation), the opposition protests when $(p+f)(q)(-B-c) + (p+f)(1-q)(B-c) > (p+f)(-B)$. Solving this equality for *q* yields the same $q*$ when $p > -f$, which is always true by construction.

This yields Lemma 1: The opposition will protest when $$q < q* = 1-c/(2B)$$, regardless of the incumbent's choice of F.

If $q < q*$, the incumbent will engage in manipulation when $(p+f)(q)(B-c) + (p+f)(1-q)(-B-c) + (1-(p+f)(-B) > (pq)(B-c)+(p(1-q)(-B-c) + (1-p)(-B)$, and hold a clean election otherwise. The incumbent will thus play F = 1 when $q > c/2B$. There are, as a result, two important threshold values for *q*. First, $q_p = 1 - (c/2B)$. When $q < q_p$, the opposition will stage post-election protest. Second, when $q > q_f = c/2B$, the incumbent will engage in election manipulation. As a result, there are four possible equilibria to the game. These are:

1) Democratic acceptance: For $c/2B > q > 1-(c/2B)$, F = 0 and Ch = 0.
2) Unchallenged manipulation:  For $q > 1 - (c/2B) > c/2B$, F = 1 and Ch = 0.
3) Confrontation: For $1 - (c/2B) > q > c/2B$, F = 1 and Ch = 1.
4) Undemocratic challenge: For $1 - (c/2B) > c/2B >q$, F = 0 and Ch = 1.

As the inequalities show, the equilibrium outcome depends on the relationship between *q*, the incumbent's probability of defeating a protest movement and the ratio between the cost of conflict *c* and the degree of polarization *B*. Iincreasing distance between the groups represented by the incumbent and the opposition increases the size of *B*; holding *c* constant, shifting values of *B* affect the range of *q* for which each equilibrium is possible. At low levels of polarization, each player is more willing to give up power to the other. As such, the risk of a costly conflict prevents manipulation by the incumbent and undemocratic attempts to challenge a clean election by the opposition. At very low values of *B*, this democratic equilibrium holds for all values of *q*. 
...
Whether articulated formally or informally, this theory yields two hypotheses.


>H1: Post-election protest will increase as polarization increases.

>H2: Election manipulation will increase as polarization increases.

As the figure illustrates, the brief formal model suggests four types of outcome that are influenced by polarization: 'successful deterrence' in which elections are clean and are not protested, mutual conflict in which elections are manipulated and protested, and two asymmetric outcomes. 'Crying wolf' occurs when the opposition chooses to protest a clean election, and 'undemocratic dominance' occurs when a manipulated election is unprotested. While the formal model presented here is for illustrative purposes, and does not seek to capture a range of features likely to exist in the real world (e.g. uncertainty about the incumbent's likelihood *q* of winning the post-election conflict, or the incumbent's ability to select an optimum level of manipulation), it can still aid in making predictions. 

If we assume that incumbents are more-or-less evenly distributed in terms of their ability to defeat protest movements at a given moment in time, increasing polarization narrows the range of incumbents who will hold clean elections and not be challenged afterward. As polarization increases, two effects take hold: weaker incumbents become more willing to gamble on manipulation to maintain power, and weaker opposition groups become more willing to protest to gamble on gaining it. This yields four hypotheses.

>H3: A 'successful deterrence' outcome, where clean elections go unprotested, is more likely as polarization declines.

>H4: The 'undemocratic dominance' outcome, where manipulated elections go unprotested, is more likely as polarization increases.

>H5 ("Crying wolf"): Post-election protest after clean elections will be more likely as polarization increases.

>H6 ("Mutual conflict"): Post-election protests following manipulated elections will be more likely as polarization increases.
...
For the first set of models, which focus on the likelihood of post-election protest, the level of election integrity is also an important explanatory variable; the theory expects that low election integrity will be associated with a greater likelihood of protest when political polarization is low, but not when it is high. *Election integrity index* is used as an explanatory variable in these models.

## Control variables

Control variables are included to account for possible confounders of polarization, protest, and election integrity. Since protest is more likely when the winner's margins of victory are low [@magaloni2006], I include *winner's vote-share* in the models. This variable is derived from V-Dem variables 'v2elvotlrg' and 'v2ellovtlg' for executive and legislative elections, respectively, with updates to missing values by the author. *Executive* and *off-schedule elections*, and whether or not the incumbent executive faced *term-limits* are coded using binary variables from NELDA. To account for the possibility that the degree of physical repression can affect protest risk [@Davenport2019], a measure of *physical integrity* rights from V-Dem is included ('v2x_clphy'). Since protest may be more likely in places with greater population density, the *urban population share* is also included, using a measure taken from the World Bank [@worldbank_urban]. Finally, to account for general political openness and competition, I include a modified version of V-Dem's additive polyarchy index ('v2x_api'). This index incorporates measures of civil society openness and capacity to organize, media freedom and freedom of expression, the extent of suffrage, and the integrity of elections; to avoid built-in multicollinearity, I subtract the electoral-integrity component of the index. I label this variable *democratic openness*. Lastly, some models include the number of *pre-election protests* recorded in ECAV as a control variable.
...
Table \@ref(tab:table-protest) reports the results of the models of post-election protest, for tests of Hypothesis 1. Models 1, 2, and 4 include all observations. Model 1 excludes a control variable for election integrity. As a more restrictive test, Model 3 includes only those observations where the incumbent initially claimed victory. Model 4 includes all observations, along with fixed effects by country. In each of these specifications, affective polarization is associated with an increase in the number of expected post-election protests. The effect is substantively sizeable as well as statistically significant. Taking the coefficient for polarization reported in Model 4 (the fixed-effects model), a one-unit increase in affective polarization is associated with a 97% increase in the expected number of post-election protests, on average. A one-unit increase is less than one standard deviation of the variable (1.22).


Next, Table \@ref(tab:manipulation-table) shows the results of multilevel models of overall election integrity, testing H2. Both models include all available observations; Model 6 includes the number of pre-election protests as a control variable. In both models, affective polarization is negatively associated with overall election integrity. As in the prior set of models, this effect is substantively large. A one-unit increase in polarization is associated with -0.14 change in overall election integrity, or about one-half of the standard deviation of that variable (sd = .31). The average change is roughly equivalent to the change in election integrity in Russia between 2003 (0.476) and 2012 (0.327).
...
modelsummary::modelsummary(models, stars = T, title = "Multilevel regression models of overall election integrity (random intercepts by country)",
                           coef_rename = 
                             c("legcon.1lag" = "Legislative constraints (lag)" , "lji.lag" = "Latent judicial independence (lag)" , "v2elintmon" = "Intnl. election observers",  "gdpgro.1lag" = "GDP growth (lag)",  "presidential" = "Presidential", "inc.termlimit.num" = "Incumbent term limited",  "urban.pct" = "Urban percent",  "v2cacamps.1lag" = "Affective polarization (lag)", "polyarchy_noelex.1lag" = "Democracy index (lag)"))
```

Hypotheses 3 through 6 are tested refer to specific outcomes of election integrity and protest, referring to the illustrative equilibria depicted in Figure...  Models 8 and 9 are logit models of unprotested elections (clean and manipulated, respectively). As predicted, we observe in Model 8 that increases in affective polarization are associated with a decrease in the likelihood of the normatively ideal outcome\textemdash a clean election which yields no protests. The effect size is small but not meaningless; each one-unit increase in affective polarization reduces the likelihood of a clean, unprotested election by about 5%. Likewise, exponentiating the coefficient on *affective polarization* from Model 9 shows that a one-unit increase in polarization makes the 'undemocratic dominance' outcome\textemdash a manipulated, unprotested election\textemdash about three percent more likely.
...
```{r}
model_wolf <- glm.nb(protested_clean_count ~  winner.share.upd + presidential + physinteg.1lag +  urban.pct +  unscheduled_election  +v2cacamps.1lag, data = vdem_data)
#summary(model_wolf)

model_success <- glm(clean_no_protest ~ winner.share.upd + presidential + physinteg.1lag +  urban.pct +  unscheduled_election  +v2cacamps.1lag, data = vdem_data)
#summary(model_success)

model_dominance <- glm(rigged_no_protest ~ winner.share.upd + presidential + physinteg.1lag +  urban.pct +  unscheduled_election  +v2cacamps.1lag, data = vdem_data)
#summary(model_dominance)

model_conflict <- glm.nb(mutual_conflict ~ winner.share.upd + presidential + physinteg.1lag +  urban.pct +  unscheduled_election  +v2cacamps.1lag, data = vdem_data)
...
models[["9"]] <- model_dominance
models[["10"]] <- model_wolf
models[["11"]] <- model_conflict


rows <- tribble(~term, ~Deterrence, ~Dominance, ~Crying_wolf, ~Conflict,  
                "", "Successful Deterrence", "Undemocratic dominance", "Crying wolf (n)", "Mutual conflict (n)")
...
Turning to the negative binomial models of protest count...
